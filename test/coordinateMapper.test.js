...
import { DISKS, DESIGN_VIEW, SLOTS, HYSTERESIS_DEG } from '../src/config.js';
...
describe('coordinateMapper — ACTIVE / zone mapping (left disk = chords)', () => {
  ...
  it('ACTIVE(k) → REST when returning to deadzone sets changed=true', () => {
    ...
    const m2 = createMapper({ disks: DISKS });
...
describe('coordinateMapper — left/right disk split (§2.1, mirrored view)', () => {
  ...
  it('a tip in the screen-RIGHT half drives R; L stays REST', () => {
    const mapper = createMapper({ disks: DISKS });
    const res = settle(mapper, [tipAt(DISKS.R, slotCenterDeg(4), midR)]);
  ...
  it('two tips (one per half) drive both disks independently', () => {
    const mapper = createMapper({ disks: DISKS });
    const tips = [
      tipAt(DISKS.L, slotCenterDeg(1), midR),
      tipAt(DISKS.R, slotCenterDeg(5), midR),
    ];
