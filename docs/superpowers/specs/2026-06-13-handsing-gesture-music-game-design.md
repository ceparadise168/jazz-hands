...
- **排列模式(2026-06-14 UI 可切,3 種)**:`thirds`(2D 三度,預設,移動不經過其他鍵,kind:pads)、`row`(單排鍵盤,直覺但會經過中間音,kind:pads)、`pizza`(扇形圓盤 + 中心休息區,kind:disk,用 createDiskMachine)。`melodyGeom(mode)` 解析幾何;mapper/renderer 依 `kind` 切換機制(disk↔pad)與繪製(drawDisk↔drawKeyboard),`setKeyboard` 執行期切換。**dwell 差異**:和弦盤(machineL)不帶 dwell = 指到立即發聲;melody pizza 帶 dwell = 跟琴鍵同款過濾(三種排列靈敏度一致)。
