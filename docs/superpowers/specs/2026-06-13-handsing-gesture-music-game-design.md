...
| 右手 | **單排彩虹琴鍵** = **音階(旋律)** | 指到彩色音(in-shape)就響、離開就停;鍵間留間隔、抬手跳音不刮中間〔2026-06-13:圓盤→演奏線→單排 in-shape 彩虹〕 |
...
**右手(旋律)行為 — 單排彩虹琴鍵 · in-shape〔2026-06-13 迭代:圓盤 → 演奏線 → 單排 in-shape〕**:
> 旋律改為螢幕右側「一排彩虹琴鍵」(C D E F G A B = 紅橙黃綠藍靛紫)。機制是最直覺的「指到哪顆彩色就響哪顆」:
- 食指尖在某鍵**形狀內**(`keyAtPoint`)→ `ACTIVE`:發那個音(attack + sustain);在鍵間**間隔**或鍵帶**上下外** → `REST`(靜音)。
- 鍵間留 `gap` 間隔 = 相鄰音的靜音緩衝;要跳音(如 C→E)把手**抬離鍵帶**(上 / 下)= 靜音,移過去再放回 → 不刮中間音。
- 邊界遲滯:已在某鍵時以擴張邊界(`KEY_HYSTERESIS_PX`)判斷是否仍在,消除鍵緣抖動進出。
- 兩手仍都靠食指尖單點、不辨識手勢;分工不變(螢幕左半圓盤和弦、右半琴鍵旋律)。
- 輸出沿用 `{state, zone}`(app 觸發 diff 不變)+ `aim`(= 所在鍵,供 renderer 高亮)。
- 取捨:in-shape 比演奏線更直覺,但沿鍵快滑(級進)相鄰音會短暫掃到;靠間隔 + 抬手跳音化解,對「邊唱邊點」足夠。
...
### 3.1 右手 · 大調全音階單排(7 鍵,預設)
- 單排 7 鍵 **C D E F G A B**(左→右音高遞增,一個八度的完整七音):
  `C4 D4 E4 F4 G4 A4 B4` → MIDI `[60, 62, 64, 65, 67, 69, 71]`(`musicEngine.noteForSlot(0..6)`)
- 含 Fa/Ti,能彈需要 4 度 / 7 度的完整旋律(2026-06-13:五聲→補成完整七音→再由 8 鍵收成 7 鍵單排彩虹)。
...
- **右手(旋律)〔2026-06-13 單排彩虹 in-shape〕**:螢幕右側一排 7 鍵 **C D E F G A B**,各為彩虹色(紅橙黃綠藍靛紫);暗鍵仍顯色以利「看顏色瞄準」,發聲鍵該色爆亮 + glow;游標在鍵內染該鍵色發光、在間隔 / 帶外為白色空心(提示沒聲音)。鍵為**近正方形 pad**(高度大減 → 縮小「離開鍵」所需上下擺動、利於連續演奏不手酸;參考 MIDI pad 佈局)、間隔加大(gap 40)。整排須在螢幕中線右側,否則最左鍵被左手搶。
