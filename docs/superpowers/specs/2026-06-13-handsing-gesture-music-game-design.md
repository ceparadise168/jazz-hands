...
- **排列模式(2026-06-14 UI 可切,3 種)**:`thirds`(2D 三度,預設,移動不經過其他鍵,kind:pads)、`row`(單排鍵盤,直覺但會經過中間音,kind:pads)、`pizza`(跟和弦盤一樣的扇形圓盤 + 中心休息區,kind:disk,用 createDiskMachine、無 dwell)。`melodyGeom(mode)` 解析幾何;mapper/renderer 依 `kind` 切換機制(disk↔pad)與繪製(drawDisk↔drawKeyboard),`setKeyboard` 執行期切換。
...
- **頂部玻璃控制列**:LIVE / 名稱 / 音階 preset / 調 / **排列(三度 / 鍵盤 / 披薩)** / 伴奏律動 + BPM / 音色 / **換音靈敏** / **同音靈敏**。
